...
template<typename T>
class LinkedList : DerivedFrom<T, LinkedListNode<T> > {
public:
...
struct LinkedType : public BASE, public LinkedListNode<BASE>  {
	template<typename return_type, typename... params>
	LinkedType(params... xs) : BASE(xs...) {
	}

	LinkedType(const BASE & obj) : BASE( static_cast<const BASE &>(obj) ) {
	}

	LinkedType(const LinkedType & obj) : BASE( static_cast<const BASE &>(obj) ) {
	}

	LinkedType & operator=(const LinkedType & obj) {
		BASE::operator=( static_cast<const BASE &>(obj) );
		return *this;
	}

	bool operator==(const LinkedType & obj) const {
		return BASE::operator==( static_cast<const BASE &>(obj) );
	}
};
