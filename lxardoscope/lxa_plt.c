...
{
  	int trig_pos=0;
	int disp_size=HOR.T_scale*BASE.sampling_rate/1000+1;
					// cannot change during execution
  	float sampling_rate=BASE.sampling_rate;	//cannot change during execution
...
// note that these are the numbers of chunks, not indices
	int rch=BASE.rchunk;	// number of chunks read by lx_run
	int cch=BASE.cchunk;	// number of chunks copied previously
	int tch=BASE.tchunk;	// number of chunks in tbuf (from 0 to 63)
...
// the first sample of the new set. 
	...
	BASE.tchunk=BASE.tchunk+nch;
	BASE.cchunk=BASE.cchunk+nch;
	datend=BASE.tchunk*TBUF_CHUNK_SIZE;
	}
	...
	BASE.tchunk=tch-ech+nch;
	BASE.cchunk=BASE.cchunk+nch;
	datend=BASE.tchunk*TBUF_CHUNK_SIZE;
	BASE.trig=BASE.trig-ech*TBUF_CHUNK_SIZE;
	}
	else {
		printf("illegal condition (2)\n");
 		return 0; 
	}
	if(BASE.trig<0) BASE.trig=0;
    	switch (HOR.trig_enb)  {
     		case TRIG_OFF:       
				if(ds>datend) {
					ds=datend;
...
//
	...
	BASE.trig=trig_pos;
	toffset=HOR.T_pos*sampling_rate/1000.0;
	BASE.delta=delta*1000.0/sampling_rate;
	if(HOR.trig_chan) {
		for (cont=0;cont<disp_size;cont++) {
  	   		datx[0][cont]=(cont+toffset-0.5)*1000.0/sampling_rate-BASE.delta;
  	   		datx[1][cont]=(cont+toffset)*1000.0/sampling_rate-BASE.delta;
  		}
	}
	else {
		for (cont=0;cont<disp_size;cont++) {
  	   		datx[0][cont]=(cont+toffset)*1000.0/sampling_rate-BASE.delta;
  	   		datx[1][cont]=(cont+toffset+0.5)*1000.0/sampling_rate-BASE.delta;
		}
	}
...
// data is from previous sweep 
    ...
    switch (HOR.trig_enb)  {
     case TRIG_OFF:   
		...
		for (ch=0;ch<2;ch++) {
			for (cont=0;cont<ds;cont++)
				daty[ch][cont]=tbuf[ch][cont+BASE.disp_start];
			}
		break;	
	case TRIG_ON:
	if(BASE.disp_start+toffset<0) toffset= -BASE.disp_start;
	if(BASE.disp_start+toffset+ds>BASE.datend) ds=BASE.datend-BASE.disp_start-toffset;
	if(HOR.trig_chan) {
		for (cont=0;cont<disp_size;cont++) {
  	   		datx[0][cont]=(cont+toffset-0.5)*1000.0/sampling_rate-BASE.delta;
  	   		datx[1][cont]=(cont+toffset)*1000.0/sampling_rate-BASE.delta;
		...
		for (ch=0;ch<2;ch++) {
			for (cont=0;cont<ds;cont++)
				daty[ch][cont]=tbuf[ch][cont+BASE.disp_start+toffset];
		}
	}
}
///////
BASE.disp_actual=ds; // save; needed for pointer in cb.c
